...
/* ---------------------------------------------------------
   工具：從 textarea 內容抓體重 / 身高並計算 BMI
--------------------------------------------------------- */
function computeBMIFromText(text) {
  const weightMatch = text.match(/體重：\s*([\d.]+)\s*KG/);
  const heightMatch = text.match(/身高：\s*(\d+)\s*CM/);
  ...
  const { bmi, result } = manualData.bmiCalculator(weight, height);
  return { bmi, result };
}

/* ---------------------------------------------------------
   getManualCompletion：依目前輸入決定「這一步」的建議
--------------------------------------------------------- */
...
export function initAISuggestion(textarea, overlay) {
  ...
  textarea.addEventListener("input", () => {
    ...
    if (manualResult && renderManualCompletion(text, overlay, aiRef, manualResult)) {
      aiRef.activeIndex = 0;
      return;
    }
